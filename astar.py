...
def heuristic(u, v, G):
    ...
    # Return Euclidean straight-line distance
    return ((x1 - x2) ** 2 + (y1 - y2) ** 2) ** 0.5

def astar_nearest_hospital(G, source, hospital_list):
    # Convert graph to undirected so we ensure full reachability
    ...
    for node, name in hospital_list:
        try:
            # Use A* to compute shortest path length from source to each hospital
            ...
            # If this is the smallest distance so far, update results
            ...
            # If there’s an issue, log the error (e.g., disconnected nodes)
